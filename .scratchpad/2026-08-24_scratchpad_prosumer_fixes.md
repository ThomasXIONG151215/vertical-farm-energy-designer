...
| ✅ CI pipeline fix | completed | Main repo CI had 27 runs, every one failing since 2026-04-09, never green. Root causes: (a) `pip install -r requirements.txt` referenced a never-existing file → ci.yml now `pip install -e ".[dev]"` + pyproject.toml `[tool.hatch.version] fallback-version = "2.0.0"` (no git tags); (b) lint never passed — black reformatted 26/35 files, 16 unused imports removed, 2 empty f-strings fixed, pins black==23.12.1/flake8==7.0.0; (c) test job ValueError 'Missing column timestamp' — weather CSVs are Git LFS pointers, checkout now `lfs: true`. Result: run 32799333443 success — Lint 34s ✓, Tests 3.10/3.11/3.12 ✓ (3m52s–4m16s); local pytest 226 passed / 1 failed (pre-existing network-only test_weather_no_cache). Commits aa8ad59, 45abc59, f8969e3 pushed to origin/main. |
...
### CI pipeline fix (2026-08-25) — summary

- **Trigger**: main repo CI showed all runs failing — 27 runs, every one since 2026-04-09, never green.
- **Root cause (a) — install**: `pip install -r requirements.txt` failed because requirements.txt never existed (project uses pyproject.toml). Fixed ci.yml to `pip install -e ".[dev]"`; added `[tool.hatch.version] fallback-version = "2.0.0"` to pyproject.toml (repo has no git tags, hatch-vcs requires a fallback). Cache key now hashes pyproject.toml.
- **Root cause (b) — lint**: `black --check vfed/` failed — 26/35 files were never black-formatted; flake8 had 158 issues. Ran `black vfed/` (reformatted 26 files), removed 16 unused imports (F401), fixed 2 empty f-strings (F541), fixed E231/W291. Pinned black==23.12.1 / flake8==7.0.0 in the lint job for determinism; flake8 ignore list: E501,W503,W504,E114,E116,E241,E261,F841. Removed the no-op `|| true` format step in the test job.
- **Root cause (c) — tests**: "Run tests" failed with `ValueError: Missing column 'timestamp'` — data/weather/*.csv are Git LFS pointer files (.gitattributes tracks *.csv via LFS) and actions/checkout does not fetch LFS by default. Added `lfs: true` to both checkout steps.
- **Commits**: aa8ad59 (Phase 3 S1/S3), 45abc59 (ci: fix CI pipeline), f8969e3 (ci: fetch Git LFS weather data on checkout). All pushed to origin/main.
- **Result**: CI run 32799333443 conclusion=success — Lint ✓ (34s), Test 3.10/3.11/3.12 ✓ (3m52s–4m16s). Local pytest: 226 passed / 1 failed (pre-existing network-only test_weather_no_cache, unchanged).
- **Note**: vfed-web and engine untouched; black reformat is formatting-only (pytest verified no behavior change).
