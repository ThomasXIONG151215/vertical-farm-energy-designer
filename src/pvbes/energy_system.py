...
class EnergySystem:
    ...
    def simulate_performance(self, x, weather: Dict[str, np.ndarray],
                             load: np.ndarray) -> Dict[str, np.ndarray]:
        A_pv, E_bat = float(x[0]), float(x[1])
        load = np.asarray(load, dtype=float)
        pv_power = self.pv.calculate_pv_output(weather, A_pv)
    ...
    def calculate_metrics(self, x, weather: Dict[str, np.ndarray],
                          load: np.ndarray) -> Dict[str, float]:
        A_pv, E_bat = float(x[0]), float(x[1])
        load = np.asarray(load, dtype=float)
        perf = self.simulate_performance(x, weather, load)
        ...
        annual_om = capital_cost * self.maintenance + bat_cost["energy_capacity_kwh"] * \
            self.battery.c_energy * 0.0  # maintenance folded into fraction above
        annual_om = capital_cost * self.maintenance

        tcost = self.tariff.annual_cost(perf["grid_import"], perf["grid_export"], hours)
        net_grid_cost = tcost["net_grid_cost"]

        # Baseline: same load served entirely from grid (no PV/battery).
        baseline_cost = float(np.sum(load * self.tariff.price_array(hours)))
