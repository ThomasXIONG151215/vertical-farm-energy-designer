...
def pso(func, lb, ub, ieqcons=[], f_ieqcons=None, args=(), kwargs={}, 
        ...
        minstep=1e-8, minfunc=1e-8, debug=False, center=None):
    """
    ...
    # 新增：若调用者给了 center，则生成“满足和=target”的种群
    if center is not None:
        target = center.sum()                   # 目标总和
        x = np.empty((S, D))
        for i in range(S):
            # 先以 center 做高斯扰动，再投影到超平面
            xi = center + 0.2 * (ub - lb) * np.random.randn(D)
            xi = np.clip(xi, lb, ub)
            delta = target - xi.sum()
            # 均匀分摊偏差并再次裁剪
            for _ in range(3):
