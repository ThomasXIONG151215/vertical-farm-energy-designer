...
class DynamicSHR:
    ...
    shr_min: float = 0.45
    shr_max: float = 1.00
    # Transition band (degC) over which SHR blends 1.0 -> physical BF-ADP
    # value as T_adp approaches T_dp from below.  Numerical smoothing ONLY
    ...
    def calc_shr(self, T_return: float, RH_return: float, T_supply: float) -> float:
        ...
        # Numerical smoothing (NOT a physics change): the BF-ADP value is
        # already continuous at T_dp (dW->0 -> SHR->1.0), but blending over
        # shr_transition_band removes the 1-dt knife-edge flip and damps
        # compressor duty chatter near the crossing.  w=1 at the band edge
        # (pure physical SHR), w=0 at T_dp (SHR=1.0, no condensation).
        band = max(self.shr_transition_band, 1e-6)
        if T_dp - band < T_adp < T_dp:
            w = (T_dp - T_adp) / band
            shr = w * shr + (1.0 - w) * 1.0
            shr = max(self.shr_min, min(self.shr_max, shr))
        return shr
