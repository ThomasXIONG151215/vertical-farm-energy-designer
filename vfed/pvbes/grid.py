...
@dataclass
class Tariff:
    hourly_prices: List[float] = field(default_factory=lambda: [0.10] * 24)  # index = hour-of-day
    ...
    def annual_cost(
        self, grid_import: np.ndarray, grid_export: np.ndarray, hours: np.ndarray, dt: float = 1.0
    ) -> Dict[str, float]:
