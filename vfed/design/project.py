...
class DesignProject:
    ...
    def from_dict(cls, d: dict) -> "DesignProject":
        ...
        def sub(target, data, *, yaml_path: str,
                has_nested_capital: bool = False):
        ...
        def _tariff(d: dict) -> TariffConfig:
            ...
            if "peak_price" in d:
                _legacy_keys = {"peak_price", "normal_price", "valley_price",
                                "peak_hours", "valley_hours", "export_price"}
                ...
                _require_number(
                    ["peak_price", "normal_price", "valley_price", "export_price"],
                    d, "tariff")
        ...
        sp_cfg = sub(SetpointConfig, d.get("setpoints", {}),
                     yaml_path="setpoints")
        _require_number(
            ["T_light", "T_dark", "RH", "co2_ppm", "crop_cycle_days"],
            sp_cfg, "setpoints")
        ...
        pv_cfg = sub(PVConfig, d.get("pv", {}), yaml_path="pv",
                     has_nested_capital=True)
        ...
        if _heat_mode is not None and _heat_mode not in ("heat_pump", "resistive"):
            raise ValueError(
                f"hvac.heat_mode must be one of "
                f"heat_pump|resistive, got {_heat_mode}")
        ...
        if _method in ("daily_per_period", "per_plant_per_period"):
            ...
            _need = ("daily_water_L_period" if _method == "daily_per_period"
        ...
        led_cfg = sub(LEDConfig, d.get("led", {}), yaml_path="led",
                      has_nested_capital=True)
        _require_number(
            ["power_w", "light_start_hour", "photoperiod_hours", "heat_fraction",
             "efficacy", "ppfd_target", "covered_area"],
            led_cfg, "led")
        if (led_cfg.get("auto_deduce", True)
                and led_cfg.get("power_w") not in (None, 1300.0)):
            import warnings as _w
            _w.warn(
                "led.auto_deduce=True ignores led.power_w (recomputed as "
                "ppfd_target*covered_area/efficacy). Set led.auto_deduce=False "
                "to make led.power_w effective.", UserWarning, stacklevel=2)
        _spectrum = led_cfg.get("spectrum")
        if _spectrum is not None and _spectrum not in (
                "white", "rb_3to1", "rb_4to1", "rb_2to1"):
            raise ValueError(
                f"led.spectrum must be one of "
                f"white|rb_3to1|rb_4to1|rb_2to1, got {_spectrum}")
        ...
        if _objective is not None and _objective not in (
                "lcoe", "kwh_per_kg_fresh", "cost_per_kg_fresh"):
            raise ValueError(
        ...
        for _name, _rng in _pr.items():
            ...
            for _i, _v in enumerate(_rng):
                if isinstance(_v, bool) or not isinstance(_v, (int, float)):
                    raise ValueError(
                        f"space.parameter_ranges['{_name}'][{_i}] must be "
                        f"numeric, got {_v!r}")
        ...
        _require_number(
            ["equipment_power_w", "interest_rate", "exchange_rate",
             "pv_area_m2", "battery_kwh"],
            d, "project")
        ...
        if ("lat" not in _site_d or "lon" not in _site_d) and not _site_d.get("city"):
            _w.warn(
                ...
                "a silently wrong climate.", UserWarning, stacklevel=2)
        if "name" not in d:
            _w.warn(
                "project 'name' missing — using 'unnamed'. Set name to "
                "identify this design in outputs.", UserWarning, stacklevel=2)
        ...
        if _cur and _cur != "USD" and _fx == 1.0:
            _w.warn(
                ...
                f"keep currency='USD'.", UserWarning, stacklevel=2)

        site_cfg = sub(SiteConfig, d.get("site", {}), yaml_path="site")
        _require_number(
            ["lat", "lon", "tz_hours", "tilt", "azimuth", "year"],
            site_cfg, "site")
        ...
        return cls(
            ...
            equipment_capital=CapitalCostConfig(**sub(
                CapitalCostConfig, d.get("equipment_capital", {}),
                yaml_path="equipment_capital")),
            envelope_capital=CapitalCostConfig(**sub(
                CapitalCostConfig, d.get("envelope_capital", {}),
                yaml_path="envelope_capital")),
            pump_capital=CapitalCostConfig(**sub(
                CapitalCostConfig, d.get("pump_capital", {}),
                yaml_path="pump_capital")),
