...
class SimulationResult:
    ...
    Units:
        summary: energy values in kWh (annual) / kWh/kg (specific, fresh
        basis), masses in kg (dry basis for harvest), all cost values in the
        project currency given by ``currency`` (see ``exchange_rate`` for
        USD conversion).
    """
    ...
    # ── summary KPIs ──────────────────────────────────────────────
    # summary keys (units): annual_energy_kwh (kWh/yr), specific_energy_kwh_per_kg
    # (kWh/kg fresh), annual_harvest_kg (kg dry), annual_water_m3 (m³/yr),
    # lcoe (currency/kWh), pv_generation_kwh / grid_import_kwh /
    # grid_export_kwh (kWh/yr)
    ...
    # ── auto-sized equipment (P4-9): nameplate values that engine.run()
...
# ---------------------------------------------------------------------------
# self-check (run: python -m vfed.design.result)
# ---------------------------------------------------------------------------
def _demo():
