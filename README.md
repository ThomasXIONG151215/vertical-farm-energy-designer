...
Extract and process weather data for locations defined in the city coordinates JSON file:
...
# Process weather data for specific cities
C:/Users/Administrator/AppData/Local/Programs/Python/Python311/python.exe weather_processor.py --cities shanghai dubai harbin

# Process using a custom coordinates file
C:/Users/Administrator/AppData/Local/Programs/Python/Python311/python.exe weather_processor.py --coordinates_file "path/to/your/coordinates.json"
```

### Customizing City Coordinates

To add custom locations, edit the `weather/city_coordinates.json` file following this format:
...
Each city entry requires:
- A unique key (lowercase, no spaces)
- Full name (for display purposes)
- Latitude and longitude coordinates
- Timezone in IANA format (e.g., "America/New_York", "Asia/Shanghai")

The weather processor:
1. Downloads historical weather data for each location using the Open-Meteo API
...
OpenCROPS/
  ...
  │   ├── city_coordinates.json # City location data
...
要添加自定义位置，请按照以下格式编辑`weather/city_coordinates.json`文件：
...
- 完整名称（用于显示）
- 纬度和经度坐标
- IANA格式的时区（例如，"America/New_York"，"Asia/Shanghai"）

天气处理器：
1. 使用Open-Meteo API下载每个位置的历史天气数据
...
在人工光植物工厂(PFALs)中，光周期指植物暴露在光照和黑暗中的日常循环。与传统农业不同，PFALs使用LED照明系统创建完全可定制的光周期，不受外部天气或季节条件的影响。这种控制允许：

1. **优化植物生长**：不同作物对光周期有不同的最佳生长要求
2. **生产调度**：操纵光周期可以加速或减缓生长周期
3. **能源管理**：将照明周期与能源可用性或价格对齐

OpenCROPS中的默认配置使用标准的16小时光照期和8小时黑暗期，这对于PFAL中种植的许多叶菜类作物来说是典型的。然而，能源优化的关键变量是这个光周期的*开始时间*，因为它决定了能源消耗模式如何与光伏发电和电网定价对齐。

### IDF文件和PFAL建模

`idfs`目录包含Energy Plus输入数据文件(IDF)，这些文件模拟了论文中描述的示例PFAL。这些文件将真实PFAL的物理和操作参数转换为Energy Plus可以模拟的格式：

- **light_schedule_XX.idf**：每个文件代表不同的光周期开始时间（XX = 小时00-23）
- **LIGHT_SCHEDULE_XX_XX.idf**：具有特定开始和结束时间的其他变体
- **Template.idf**：PFAL结构的基本模板

IDF文件包括以下规格：

1. **建筑围护结构**：与示例PFAL匹配的尺寸、材料、热特性
2. **暖通空调系统**：从实际测量中获得性能特征的空调单元
3. **照明系统**：具有功率额定值和散热特性的LED灯具
4. **设备负载**：其他耗能组件，如风扇和泵
5. **时间表**：所有系统的操作时间，特别是照明时间表

在进行模拟时，这些IDF文件与特定位置的天气数据相结合，生成能源消耗曲线，这些曲线考虑了内部负载（照明、设备）和外部因素（温度、太阳辐射）。

### 示例PFAL特性

IDF模型基于对一个运行了整年的PFAL设施的测量。主要特点包括：

- **组件级能源监测**：LED照明、空调单元、新风单元(FAU)和风机过滤单元(FFU)
- **季节变化**：空调能耗在不同季节之间变化>140%，而照明保持相对稳定
- **COP测量**：空调效率显示季节性变化（2.57-3.94）
- **LED照明模式**：持续稳定的能源消耗（10.2-12.7 kWh/天），带有周期性调整

这些测量用于验证EnergyPlus模型，确保在不同气候条件和操作时间表下能准确表示能源消耗模式。
