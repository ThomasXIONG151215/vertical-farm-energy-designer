...
| Plant transpiration adds latent load | 6 configurable transpiration methods — direct-set (constant / daily / per_plant) and model-calculated (VPD / stomatal / Van Henten) |
...
Runs the building simulation for a single configuration and reports annual load, biomass, and energy intensity (kWh/kg). If the project declares `pv` / `battery`, it also reports PV generation and grid import/export.

### Parametric Sweep (PV-Battery sizing)

```bash
vfed sweep my_farm.yaml --cache weather_cache --out results.csv
```

Enumerates the parameter ranges declared under `space.parameter_ranges` (e.g. PV area × battery capacity) and returns the design that minimises the configured objective — `lcoe` (default), `kwh_per_kg_fresh`, or `cost_per_kg_fresh`. There is no separate `optimize` command; sizing is done via `sweep`.
...
| `vfed design cities` | List built-in cities |
| `vfed design tariffs` | List built-in tariffs |
| `vfed evaluate <project.yaml>` | Run the building simulation for one configuration |
| `vfed sweep <project.yaml>` | Run parametric sweep over `space.parameter_ranges` (e.g. PV area × battery capacity) |
...
- **transpiration** — method (constant / daily / per_plant / vpd / stomatal / van_henten)
...
- **tariff** — 24-hour price schedule (`hourly_prices`) + export price
