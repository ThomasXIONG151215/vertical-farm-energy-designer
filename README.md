...
- **growth** — Van Henten growth-model parameters (un-calibrated **tomato** coefficient; see the yield caveat under [Interpreting Results](#interpreting-results))
...
> **Yield-model caveat — read before quoting KPIs.** The Van Henten growth model still uses the **tomato** literature coefficient `c_rad_phot` (Van Henten 2003) without lettuce recalibration. Modelled annual yield is therefore about **2-4x** the 30-60 kg fresh/m²/yr reported for commercial PFAL lettuce, which makes `kwh_per_kg_fresh` (and `cost_per_kg_fresh`) **optimistic by roughly half**. These KPIs are meaningful for comparing VFED design variants against each other, but recalibrate `c_rad_phot` before comparing them with external facility data. See the calibration note in `vfed/plants/van_henten.py`.
